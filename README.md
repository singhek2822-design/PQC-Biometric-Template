...
- **Logs:** Detailed traces per module (e.g., `logs/PQC_system.log`, `logs/classical_system.log`, `logs/attack_simulation.log`, `logs/gan_attack.log`,`ckks_homomorphic_detailed_XX.log`, `comprehensive_performance_evaluation.log`, `module_lwe_detailed_analysis_XX.log`, `quantum_safe_mfa_detailed.log`)
- **CSVs:** Attack results (inversion, brute force, hill climbing, GAN) — e.g., `logs/attack_results.csv`, `logs/gan_attack_results.csv`
- **PNGs:** Attack comparison plots & unlinkability histograms — e.g., `logs/attack_comparison.png`, `logs/gan_attack_histogram.png`
